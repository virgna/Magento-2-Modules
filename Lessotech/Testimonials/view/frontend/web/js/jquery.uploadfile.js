...
define([
    ...
    if($.fn.ajaxForm == undefined) {
        //$.getScript(BASE_JS_URL + "cp/jquery.form.js");
       ...
       this.startUpload = function () {
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                ...
                else fileNameStr = '<span>'+files[i].name+'</span>';
        ...
        {
            this.statusbar = $("<div class='ajax-file-upload-statusbar' id='status-bar-count-"+obj.fileCounter+"'></div>");
            /*default - this.statusbar = $("<div class='ajax-file-upload-statusbar' id='status-bar-count-"+obj.fileCounter+"'></div>").width(s.statusBarWidth);*/
            ...
            this.del.addClass("delete-progressbar");
            ...
            var options = {
                ...
                uploadProgress: function (event, position, total, percentComplete) {
                    ...
                    if( event.lengthComputable )
                    {
                        ...
                        var completed_Kbytes =  getSizeStr(loaded);
                        var total_Kbytes = getSizeStr(total);
                        
                        // Time Remaining
                        /*var seconds_elapsed =   ( new Date().getTime() - started_at.getTime() )/1000;
                        var bytes_per_second =  seconds_elapsed ? loaded / seconds_elapsed : 0 ;
                        var Kbytes_per_second = bytes_per_second / 1000 ;
                        var KBsize = Kbytes_per_second.toFixed(2)*/
                        // Percentage.
                    }
                    if(percentComplete > 1) pd.progressbar.width(percentVal)
                    if(s.showProgress) {
                        pd.progressbar.html('<span class="progress_percent_complete">'+completed_Kbytes+'</span><span class="progress_size">'+total_Kbytes+'</span>');
                ...
                success: function (data, message, xhr) {
                    ...
                    pd.progressbar.width('100%')
                    if(s.showProgress) {
                        pd.progressbar.html('<span class="progress_percent_complete">100% Done</span><span class="progress_size">0 KB/sec</span>');
                        /*pd.progressbar.css('text-align', 'center');*/
                    }
                    pd.statusbar.hide();
                    pd.abort.hide();
